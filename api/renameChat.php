...
if (!$chatId || !$title) {
    ...
    echo json_encode([
        'error' => 'chat_id and title required'
    ]);
    exit;
}
...
try {
    ...
    // ✏️ Step 2: Update title using REAL DB id
    $stmt = $pdo->prepare("
        UPDATE sessions 
        SET title = ?
        WHERE id = ? AND user_id = ?
    ");
    $stmt->execute([$title, $session_db_id, $user['id']]);

    if ($stmt->rowCount() === 0) {
        throw new Exception("Rename failed or no changes made");
    }
